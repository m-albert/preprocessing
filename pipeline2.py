...
if os.path.exists(beadParamsFile):
    ...
    segmentedBeads = [beads.segmentBeads(
                      beads.loadFiles([os.path.join(config['beads_LC'][i],config['filePattern'] %{'t':0}),
                                       os.path.join(config['beads_RC'][i],config['filePattern'] %{'t':0})],handler.config),
...
for itp,tp in enumerate(config['tps']):
    ...
    for iviews,views in enumerate(dataFiles):
        ...
        if onlyExecutePart not in [0,1]:
            ...
            tmpims = beads.loadFiles([os.path.join(view,config['filePattern']) %{'t':tp} for view in views],handler.config)
            if opposingWeights == None: opposingWeights = fusion.calculateOpposingWeights(tmpims[0].GetSize())
            if onlyExecutePart in [1] and config['opposingFusionWithElastix']:
                opposingFusionParams = [[1.,0,0,0,1,0,0,0,1,0,0,0],fusion.registerWithElastix(tmpims[0],tmpims[1],handler.config)]
            else:
                opposingFusionParams = [alignParams[iviews][0][0],alignParams[iviews][0][1]]

            for ii in range(len(tmpims)):
                tmpims[ii] = beads.transformStack(opposingFusionParams[ii],tmpims[ii])
            ...
            tmpims = fusion.fuseStacks([tmpims[0],tmpims[1]],opposingWeights)
            tmpims.SetSpacing(stackSpacings[iviews])
        ...
        if config['processAdditionalTimepoints']:
            for ichannel in range(numberOfAdditionalChannels):
                ...
                if onlyExecutePart in [2]:
                    ...
                    tmpims = beads.loadFiles([os.path.join(addDataFiles[iviews][iview],config['filePattern']%{'t':tp}) for iview in range(len(addDataFiles[iviews]))],handler.config)
    ...
    if not handler.useExistingParameters or not handler.existingParameters.has_key(str(tp)):
    ...
    if numberOfViews > 1:
        ...
        if finalWeights == None:
            finalWeights = fusion.calculateFinalWeightMatrices(list(newAlignParams[:][1][1]),
    ...
    if not handler.config['keepTemporaryImageFiles'] or handler.config['useFileCaching']:
        ...
        for iview,view in enumerate(alignedViews):
            ...
            if onlyExecutePart not in [2] and not itp:
                tifffile.imsave(os.path.join(handler.config['pipeDir'],'opposingWeightsView%s.tif'%iview),sitk.GetArrayFromImage(sitk.Cast(opposingWeights[iview]*n.power(2,15),3)))
    ...
    if config['useFileCaching']:
        ...
        if onlyExecutePart not in [1]:
            ...
            paramUpload = handler.copyFiles([source],[target],isDownload=False)
